            ...
            <div className="w-full">
                <div className="relative">
                    ...
                    <div className={`flex flex-col ${stepCount === 0 ? '' : 'hidden'}`}>
                        <button onClick={() => { setReason('use'); setReasonFull(`I don't use the xApp anymore`); setStepCount(1) }} className="text-left my-2 first:mt-0 last:mb-0 active:font-bold focus:font-bold focus:outline-none">&rarr; I don't use the xApp anymore</button>
                        <button onClick={() => { setReason('trust'); setReasonFull(`I don't trust this xApp / Project / Developer anymore`); setStepCount(1) }} className="text-left my-2 first:mt-0 last:mb-0 active:font-bold focus:font-bold focus:outline-none">&rarr; I don't trust this xApp / Project / Developer anymore</button>
                        <button onClick={() => { setReason('push'); setReasonFull(`I get too many push notifications`); setStepCount(1) }} className="text-left my-2 first:mt-0 last:mb-0 active:font-bold focus:font-bold focus:outline-none">&rarr; I get too many push notifications</button>
                    </div>
                ...
                {stepCount > 0 &&
                    <div>
                        ...
                        {stepCount === 1 &&
                            <>
                                <textarea onBlur={(element) => { blurFunction() }} onFocus={(element) => { focusFunction(element) }} className="w-full border p-2" rows={4} onChange={(e: any) => { setExplanation(e.target.value) }} defaultValue={explanation}></textarea>
                ...
                {stepCount > 1 && reason !== 'use' &&
                    <>
                        ...
                        <div className={`flex flex-col ${stepCount === 2 ? '' : 'hidden'}`}>
                            <button onClick={() => { setIsAbuse('Yes'); setStepCount(3) }} className="text-left my-2 first:mt-0 last:mb-0 active:font-bold focus:font-bold focus:outline-none">&rarr; Yes</button>
                            <button onClick={() => { setIsAbuse('No'); setStepCount(3) }} className="text-left my-2 first:mt-0 last:mb-0 active:font-bold focus:font-bold focus:outline-none">&rarr; No</button>
                        </div>
                ...
                {stepCount > 2 &&
                    <div>
                        ...
                        <textarea onBlur={(element) => { blurFunction() }} onFocus={(element) => { focusFunction(element) }} className="w-full border p-2" rows={4} onChange={(e: any) => { setExtraInfo(e.target.value) }} defaultValue={extraInfo}></textarea>
                        <div className="flex justify-end mt-2">
